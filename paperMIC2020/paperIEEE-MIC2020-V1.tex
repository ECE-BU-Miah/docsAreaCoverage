...
Tracking a random moving target using a mobile robot is a key problem and a fundemental feature in Autonomus robots in areas such as survillance,search and rescure missions, enviromental monitoring and household cleaning to name a few \cite{kolling2006},\cite{Encarnacao2001},\cite{Ju2001},\cite{Kitowski2012},\cite{Zhang2013}. Throught the years dynamic target tracking problem has recieved profound amount of research in academia and industry.  This is mainly due to their inherent complex nonlinear dynamics. Most of the target tracking algorithms proposed in the literature either rely on A) mathematical and kinematic models of both the robot and the target \cite{Huang2008}, B) the assumption that the target or it's path are static and known in prior, and C) nonlinear control techniques or driven by large amount of mostly offline data that leads to an overwhelming degree of computational complexity \cite{KD2007},\cite{Cren2017}. In a typical tracking problem a robot is to apply a local control actions that enables the robot to maintain a relative position and orientation from the designated target while following a certain path.It can also be applied on a fleet of robots where multiple robots mainatin a geometric configuration relative to the target\cite{Oh2015-Survey},\cite{Liu2017-Modular}.
...
Reinforcement learning (RL) is an emerging field of machine learning which refers to a set of algorithms and techniques in which agent learn by interacting with the enviroment and generates it's own set of data. Reinorcement learning has long been applied on games and simulated enviroments but it has recently extended to robots in real world applications such as localization and mapping,area coverage, path planning and obstacle avoidance \cite{kober2013reinforcement},\cite{smart2002},\cite{michels2005},\cite{adekunle2016}. 

In this paper we develop a model-free actor-critic learning  approach to control a non-holonomic wheeled mobile robot.The proposed method offers many advantages over other methods mentioned in the litreature such as being completely model free and does not require any knowledge of the mathemtaical model of the robot or the moving target and without the need for large computational power. The steering angle and the linear speed of the robot referred to as the policy are determined by collecting the position and orientation of both the target and the robot. This set of information is gathered online over a finite period of time. The optimal control actions are then generated by utilizing Bellman's principal of optimality which acts as model free-reinfocment learning approach that allows the  robot to follow the path of the target while avoiding collision by maintaining a safe distance. Here we extend the work proposed by authors in \cite{miah2020} by incorporating actor neural network along with critic network to find optimal control policy for the trajectory tracking problem. This work acts as first milestone in generalizing the algorithm to solve more sophisticated problem such as coverage and mapping.  

The rest of the paper is orgnized as follows. Section II lays down the problem setting of the dynamic target tracking problem and mathemetical models of the robot and the state error. The model free actor-critic reinforcment learning algorithm and all the mathemetical equations are detailed in section III.Section IV illustrates computer simulations for different secnarios that reflects the effectivness of the proposed method followed by conlusion and future work presented in section V.      
...
with $\mathbf{u}_k^{[d]}\in\mathbb{R}^2$ being the control input vector (velocity) that defines the trajectory (considered random in this work) of the target.  In case, the robot modeled by~\eqref{eq:robotModelDT} is described by a differential drive mobile robot, then its left-wheel velocity $\nu_{L,k}$ and right-wheel velocity $(\nu_{R,k})$ are related as 
 ...
 \begin{align}
     \label{eq:stateError}
   \mathbf{e}_k = [\tilde{\rho_k},\tilde\theta_k]^T=[\sqrt{\tilde{x}_k^2+\tilde{y}_k^2},\tilde{\theta_k}]^T% = \\
   % \begin{bmatrix}
...
\begin{figure}
    ...
    \fcolorbox{blue}{gray!5}{
        \begin{adjustbox}{max width = 0.45\textwidth}
            ...
            \node [align=center, below] at (3,-2.5) {Weights \\ $\boldsymbol{\omega}_c$};
            \draw[-stealth,ultra thick](3,-2.5) to[bend left] (3,1.4);
 ...
 %
 \begin{align}
   \delta_c =\sum_{\kappa=0}^{\eta-1}\frac{1}{2}\varepsilon_{k+\kappa}^2 &=\sum_{\kappa=0}^{\eta-1}\frac{1}{2}(\boldsymbol{\omega_c}^T\tilde{\bm{\rho}}(\mathbf{z}_{k+\kappa,k+\kappa+1})-{v}_{k+\kappa})^2 \nonumber \\        	
...
For clarity, number of hidden neurons chosen in the actor neural network shown in Fig.~\ref{fig:nnActor} is $n^{'} = 2$ and the basis function vector is simply $\bm{\sigma}({\bf e}_k) = {\bf e}_k.$ In the following, we provide the computer simulation results to highlight the performance of the proposed model-free trajectory tracking approach for a mobile robot in tracking a random target while maintaining a geometric safe distance $d>0.$  
 ...
 This section adopts the theoritical results discussed in the previous section by simulating the Algorithm using a differential drive mobile robot. The proposed Actor-Critic RL algorithm is tested  utilizing Matlab software and Robot Operating system (ROS)  as a prelminary step to implement the Algorithm experimantally in real world.Here we present the performance of the proposed method and the convergence characterisitcs of the actor and critic weights. The weighting matrices are set to $\mathbf{Q} = \mathrm{diag}[0.001,0.001]$ and $\mathbf{R} = \mathrm{diag}[10^{-5}, 10^{-5}].$ % 
  \[Q=0.001 
  \begin{bmatrix}
  1       & 0   \\
  0       & 1   \\
 ...
 The actor and critic learning rates $\ell_c$,$\ell_a$ are set to  $0.0001$ and $0.01$ respectively. The sampling time $T_s$ is set to $0.001 \sec.$ The desired distance offset between the leader and the follower is set to $ d = 0.2$ [m] for the first scenario and $d = 0.5$ [m] for the second.

 
   In the first scenario the differential drive robot was initally placed at position $(x^d,y^d) = (-1,-5) $ [m] with an orientation $\theta = 0 \degree$ .The Target which is modeled as a cylinder object was initially located at a random position around  $(x,y) = (0,0)$ [m]. The simulation was run for the duration of 60 seconds.The targer is set to move following a sinusoidal path modeled as  $
 x^{[d]}(t) = \alpha (t) \in [0,10\pi], %
 y^{[d]}(t) = 5 \,\sin(\alpha t),$  and %
 $\theta^{[d]}(t) = cos(\alpha t).  %
$ In the begining of the simulation the robot starts to move away from the target according to the intial random actor weights.As the robot collects more information the robot adjust the control actions as the actor and critic weights started to converge and the robot asymptotically follows the target's trajectory. 
...
% \end{figure}  
 ...
 \subfigure[][]{%
    ...
    \includegraphics[width=0.48\textwidth,height=0.25\textheight]{figs/coppelia/secondScenario/trajectoryRandomPathDistance.eps} }
	\subfigure[][]{%    
    \label{fig:euclideanDistance}%
    \includegraphics[width=0.48\textwidth,height=0.25\textheight]{figs/coppelia/secondScenario/euclideanDistance.eps} }
\subfigure[][]{%
 ...
 \subfigure[][]{%
    ...
    \includegraphics[width=0.48\textwidth,height=0.25\textheight]{figs/coppelia/secondScenario/weightRandomPathDistance.eps} }
     \subfigure[][]{%
    \label{fig:actorWeightRandomDistance}%
    \includegraphics[width=0.48\textwidth,height=0.25\textheight]{figs/coppelia/secondScenario/weightActor.eps} }
