...
% 
 ...
 %
  \begin{figure}
   ...
   \caption{Mobile robot and its dynamic target tracking problem setup. \todo[inline]{In this figure, replace $\tilde\theta_k$ with $\theta_k^{'}$}}
...
\end{multline}
%
 where $\ell_a$ is the actor learning rate.  The complete policy iteration solution process for the leader-follower problem is detailed out in Algorithm~\ref{alg:ModelFreeTracking}. \todo[inline]{In Algorithm 1, add a line with $n=2$ (state error dimension) and $m=2$ (input dimension)} %
 %
 \begin{algorithm2e}{
...
\caption[Robot tracking a target that is following a sinusoidal trajectory.]{\comment{Robot tracking a target that is  following a sinusoidal trajectory:}{Are you using CoppeliaSim results?} 
  \subref{fig:trajectorySin} robot and target trajectories,~\subref{fig:stateErrorSin} position error $\tilde\rho_k$,~\subref{fig:controlInputsSin} linear speed and steering angle of the target, \subref{fig:criticWeightSin} critic learning weights~$\bm{\omega}_c,$ and \subref{fig:actorWeightSin} actor learning weights~${\bf W}_a$}%
  \label{fig:performanceSin}%
\end{figure*} 
%

In the second scenario, the target is set to move on a completely random trajectory starting from a random initial position $(x^{[d]},y^{[d]})\approx (1,5)~[\si{\meter}],$ where the \comment{target is now modeled as an unicycle robot}{Previously we talk about point robot model for the target but now you are using the following model for the target} described by %
%
 $x_{k+1}^{[d]} = x_k^{[d]} + T\nu_k^{[d]}\cos\theta_k^{[d]},$ %
 $y_{k+1}^{[d]} = y_k^{[d]} + T\nu_k^{[d]}\sin\theta_k^{[d]},$ %
 $\theta_{k+1}^{[d]} = \theta_k^{[d]} +T\omega_k^{[d]},$ %
 %
 where $\nu^{[d]}(t)=0.2~[\si{\meter}]$\comment{~}{What about $\omega$?} The robot initial position and orientation being $(x,y) = (0,0)~[\si{\meter}]$ and $\theta_0 = $ We intensify that the robot has no previous knowledge of the dynamic model of the moving target\comment{nor it's initial position}{Robot does need to know the target's position through the sensors}. The results obtained by the simulation are shown in Fig.~\ref{fig:performanceRandom}. The control actions of the robot are determined by the random actor weights. We clearly see tracking error converges and approaches zero as the control signal converging to values that allows target tracking. The results reveal the ability of the proposed algorithm to successfully track the dynamic moving target and adapt to the changes in the target's trajectory and learns online by trial and error in different scenarios. %
%    
 \begin{figure*}[htbp]%
 \subfigure[][]{%
...
\subfigure[][]{%
 ...
 \subfigure[][]{%
  ...
  \subref{fig:trajectoryRandom} robot and target trajectories,~\subref{fig:stateErrorRandom} position error $\tilde\rho_k$,~\subref{fig:controlInputsRandom} linear speed and steering angle of the target, \subref{fig:criticWeightRandom} critic learning weights~$\bm{\omega}_c,$ and \subref{fig:actorWeightRandom} actor learning weights~${\bf W}_a$}%
