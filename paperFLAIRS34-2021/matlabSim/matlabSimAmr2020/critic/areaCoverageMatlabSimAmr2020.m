...
z1(1,:) = [0 0 0 0 0] ;%initilize z1 with a value that is going to be replaced with a the true value.
z2(1,:) = [0 0 0 0 0] ;
z3(1,:) = [0 0 0 0 0] ;
...
for t = 0:Ts:Tf % Main timing loop 
    ...
    trackingError1(k,:) = [q1(k,1)-QD1(k,1), q1(k,2)-QD1(k,2), pi2pi(q1(k,3)-thetaPrime1(k,:))];
    trackingError2(k,:) = [q2(k,1)-QD2(k,1), q2(k,2)-QD2(k,2), pi2pi(q2(k,3)-thetaPrime2(k,:))];
    trackingError3(k,:) = [q3(k,1)-QD3(k,1), q3(k,2)-QD3(k,2), pi2pi(q3(k,3)-thetaPrime3(k,:))];
    %======================================================================
    %======================================================================  
    ...
    [Vk1,gammak1,P1,kappa1] = modelfree(q1,Ts,QD1,k,trackingError1,Ti,z1,P1,kappa1);
    P1 = P1; %saving the p form last iterations 
    %save the z vector
    z1(kappa1,:) = [trackingError1(k,:) Vk1 gammak1];
    
    [Vk2,gammak2,P2,kappa2] = modelfree(q2,Ts,QD2,k,trackingError2,Ti,z2,P2,kappa2);
    Vk2 = -Vk2;
    P2 = P2;
    z2(kappa2,:) = [trackingError2(k,:) Vk2 gammak2];
    
    [Vk3,gammak3,P3,kappa3] = modelfree(q3,Ts,QD3,k,trackingError3,Ti,z3,P3,kappa3);
    P3 = P3;
    ...
    %[Vk3,gammak3,P3,kappa] = modelfree(q3,Ts,QD3,k,trackingError3,Ti,z3,P3,kappa);
    %P3 = P3;
    %z3(kappa,:) = [trackingError3(k,:) Vk3 gammak3];
...
function [Vk,gammak, Pcalculated,kappa] = modelfree(q,Ts,QD,k,trackingError,Ti,z,P,kappa)
...
%initilze the policy
u(k,:) = -1* inv(P(4:5,4:5))*P(4:5,1:3)*trackingError(k,:)';%finding the policy;
%setting the limit for the policy
% u(k,1) = sign(u(k,1))*min(0.5,abs(u(k,1)));%limiting the linear speed 
% u(k,2) = sign(u(1,2))*min(80*(pi/180),abs(u(k,2)));%limiting the steering angle
